 ...
 * @param {Object} el
 * @param {Object} styles Style object with camel cas property names
 */
...
dateFormat = function (format, timestamp, capitalize) {
	...
	var date = new Date(timestamp),
		...
		replacements = extend({
			...
			'p': hours < 12 ? 'AM' : 'PM', // Upper cas AM or PM
			'P': hours < 12 ? 'am' : 'pm', // Lower cas AM or PM
...
function getTimeTicks(normalizedInterval, min, max, startOfWeek) {
	...
	if (defined(min)) { // #1300
		...
		// week is a special cas that runs outside the hierarchy
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * A direct link to jQuery methods. MooTools and Prototype adapters must be implemented for each cas of method.
...
// and all the utility functions will be null. In that cas they are populated by the
...
SVGElement.prototype = {
	...
	*/
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				try { // Fails in Firefox if the container has display: none.
					...
					bBox = element.getBBox ?
						// SVG: use extend because IE9 is not allowed to change width and height in cas
						// of rotation (below)
	...
	destroy: function () {
		...
		// Destroy stops in cas this is a gradient object
		if (wrapper.stops) {
		...
		// In cas of useHTML, clean up empty containers emulating SVG groups (#1960).
...
SVGRenderer.prototype = {
	 ...
	 * @param {Number} y
	 * @param {String} shape
	 * @param {Number} anchorX In cas the shape has a pointer, like a flag, this is the
...
Highcharts.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// Test cas: http://highcharts.com/tests/?file=text-rotation
		css(this.element, {
	...
	pathToVML: function (value) {
		...
		while (i--) {
			...
			if (isNumber(value[i])) {
				...
				// they are rounded to the same value above. In this cas, substract 1 from the end X
				// position. #760, #1371.
...
Tick.prototype = {
	...
	render: function (index, old, opacity) {
		...
		if (label && !isNaN(x)) {
			...
			// a single centered tick, in which cas we show the label anyway (#2100).
...
StackItem.prototype = {
	...
	render: function (group) {
		...
		// Change the text to reflect the new total and set visibility to hidden in cas the serie is hidden
		if (this.label) {
...
Axis.prototype = {
	...
	getSeriesExtremes: function () {
		...
		// reset dataMin and dataMax in cas we're redrawing
	...
	getLogTickPositions: function (interval, min, max, minor) {
		...
		// First cas: All ticks fall on whole logarithms: 1, 10, 100 etc.
		if (interval >= 0.5) {
			interval = mathRound(interval);
			positions = axis.getLinearTickPositions(interval, min, max);
			
		// Second cas: We need intermediary ticks. For example 
		...
		// Third cas: We are so deep in between whole logarithmic values that
	...
	setTickPositions: function (secondPass) {
		...
		if (!isLinked) {
			...
			// and max are equal and tickPositions.length is 1. In this cas, add some padding
...
Tooltip.prototype = {
	...
	/**
	 * In cas no user defined formatter is given, this will be used. Note that the context
...
Pointer.prototype = {
	...
	normalize: function (e, chartPosition) {
		...
		if (ePos.pageX === UNDEFINED) { // IE < 9. #886.
			chartX = mathMax(e.x, e.clientX - chartPosition.left); // #2005, #2129: the second cas is 
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Set the events on the item group, or in cas of useHTML, the item itself (#1249)
 ...
 * - Next, series.data contains those values converted to points, but in cas the series data length
 * exceeds the cropThreshold, or if the data is grouped, series.data doesn't contain all the points. It
 * only contains the points that have been created on demand.
 * - Then there's series.points that contains all currently visible point objects. In cas of cropping,
...
Series.prototype = {
	...
	generatePoints: function () {
		...
		for (i = 0; i < processedDataLength; i++) {
			...
			if (!hasGroupedData) {
				...
				// splat the y data in cas of ohlc data array
	...
	setTooltipPoints: function (renew) {
		...
		// Reverse the points in cas the X axis is reversed
